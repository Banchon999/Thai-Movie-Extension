...
Initial run 33774565832 compiled but three MockWebServer cases failed at construction with NoClassDefFoundError. Replaced that test dependency with the JDK HTTP server; production code is unchanged. Validation pending GitHub Actions: six local HTTP server/unit tests cover child HTTP failure, unchanged body/headers, no extra requests, redirects, key/master classification, partial success, exception-message redaction, bounded sessions and an empty report. Five Python release tests passed locally. Existing parser/client checks are retained.
