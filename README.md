...
**เวอร์ชัน 5 (วินิจฉัย): ตรวจ master playlist และแสดงรหัส HTTP จริงเมื่อโหลดไม่สำเร็จ — ยังไม่ยืนยันว่าแก้การเล่นได้**
...
รุ่น 5 เพิ่ม GET สำหรับตรวจ master playlist ก่อนส่งลิงก์ให้ตัวเล่น โดยใช้ User-Agent/Referer ชุดเดียวกับการเล่น
ถ้า HTTP ไม่สำเร็จ จะแสดงสถานะและชื่อโฮสต์; ถ้า HTTP 200 แต่เนื้อหาไม่ใช่ HLS จะแจ้งแยกต่างหาก
ไม่ได้เปลี่ยน Referer/Origin หรือเดาเส้นทางใหม่ และยังไม่ตรวจ variant playlist, key หรือ segment
การตรวจนี้เพิ่มหนึ่งคำขอต่อ master และตัวเล่นจะขอ master อีกครั้ง จึงยังต้องทดสอบพฤติกรรม URL แบบใช้ครั้งเดียว
